...
export function createConnection(backend: BackendConfig, signal: AbortSignal): ComfyConnection {
    ...
    const client = new Client({api_host: url.host, api_base: url.pathname.replace(/\/$/, ''), ssl: url.protocol === 'https:', user: '', fetch: authenticatedFetch, WebSocket});
    ...
    return {
        ...
        async execute(graph, outputNode) {
            ...
            try {
                ...
                promptId = submission.assert(queued).prompt_id;
            } catch (cause) {
