...
- UNIMPLEMENTED: `<output-dir>` is the catalog root. Write metadata, images, and optional workflow sidecars directly into it using the flat layout in s0004; do not append the catalog ID. Create the output directory if missing.
- UNIMPLEMENTED: PNG optimization is enabled by default under the policy in s0004. Quality settings affect newly saved images; skipped entries are unchanged. Use `--force` to regenerate existing entries with different settings.
...
- UNIMPLEMENTED: Quality bounds must be integers satisfying `0 <= MIN <= MAX <= 100`. Reject combining `--png-quality` with `--no-png-optimization`. When optimization is enabled, verify that pngquant is available before submitting jobs or resetting output; report a missing optimizer as an error.
